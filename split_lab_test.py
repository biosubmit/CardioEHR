import pandas as pd
import json
from utils import translate_column,translate_column_name
def encode_patient_ids(df,label:str):
    with open(f'patient_id_mapping_{label}.json', 'r') as f:
        id_mapping = json.load(f)
    df['Medical record number'] = df['Medical record number'].astype(str)
    df['Medical record number'] = df['Medical record number'].map(id_mapping)

    return df

def split_lab_test_csv(lab_test_df,label:str):
    # Read patient_info.csv file

    # Anonymize medical record number
    lab_test_df = encode_patient_ids(lab_test_df,label)
    
    # save to csv
    return lab_test_df

def select_df(df:pd.DataFrame):
    columns = df.columns
    drop_columns = columns[-20:] 
    print(drop_columns)
    df = df.drop(columns=drop_columns)
    df = df.drop(columns=['检查时间'])
    df = translate_column_name(df)
    print(df.head())
    return df

split_lab_test_csv(select_df(pd.read_csv('./data/final_preprocessed_data.csv')),'old').to_csv('lab_test_old.csv', index=False)
split_lab_test_csv(select_df(pd.read_csv('./data/final_preprocessed_data_new.csv')),'new').to_csv('lab_test_new.csv', index=False)
